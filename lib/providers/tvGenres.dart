...
class TvGenres with ChangeNotifier {
  List<Genre> _items = [];
  List<Genre> get items => [..._items];

  void setTvGenresFromJson(genres) {
  ...
  Color findGenreByid(int genreId) {
    ...
    String genreResult = _items.firstWhere((genre) => genre.id == genreId).genreName;

    if(genreResult == "Action & Adventure")
    {
